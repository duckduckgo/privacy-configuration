...
async function init() {
    ...
    for (legacyName in protectionsData) {
        ...
        try {
            let exceptionListData = JSON.parse(fs.readFileSync(`exception-lists/${newName}Sites.json`))
            ...
            if (arrayDifference(listData.map(obj => obj.domain), legacyData.sites)) {
                ...
                fs.writeFileSync(`exception-lists/${newName}Sites.json`, JSON.stringify(exceptionListData, null, 4))
            }
        } catch {
            if (legacyData.sites.length) {
                console.warn(`We have a missing file: exception-lists/${newName}Sites.json as the legacy feature has sites to import`);
            }
        }
    }
