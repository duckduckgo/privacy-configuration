...
async function init () {
    ...
    const newListDomains = listData.tempUnprotectedDomains.map((obj) => obj.domain)
    ...
    function getNewName (legacyName) {
        for (const newName in legacyNaming) {
    ...
    for (const legacyName in protectionsData) {
        const legacyData = protectionsData[legacyName]
        // Known deprecated items we are removing
        if (legacyName === 'do-not-track') {
            continue
        }
        if (!Object.values(legacyNaming).includes(legacyName)) {
            console.debug('mapping name missing!', legacyName)
        }
        const newName = getNewName(legacyName)
        const exceptionListName = newName.replace(/([a-zA-Z])(?=[A-Z0-9])/g, '$1-').toLowerCase()
        const newStateNaming = legacyData.enabled ? 'enabled' : 'disabled'
        ...
        try {
            const exceptionListData = JSON.parse(fs.readFileSync(`${LISTS_DIR}/${exceptionListName}-sites.json`))
