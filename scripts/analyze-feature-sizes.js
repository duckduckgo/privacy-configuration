...
const getConfigFiles = () => {
    ...
    let configFiles = fs.readdirSync(versionDir).filter(file => 
    ...
    if (options.platform) {
        configFiles = configFiles.filter(file => 
            file.includes(options.platform) || file.startsWith(options.platform)
        );
        
        if (configFiles.length === 0) {
            console.error(`No config files found for platform: ${options.platform}`);
            console.error(`Available platforms: ${fs.readdirSync(versionDir).filter(f => f.endsWith('-config.json')).map(f => f.replace('-config.json', '')).join(', ')}`);
            process.exit(1);
        }
    }
...
const main = () => {
    ...
    if (options.feature) {
        ...
        if (results.featureDetails) {
            Object.entries(results.featureDetails).forEach(([platform, feature]) => {
    ...
    results.platforms.forEach(({ platform, configSizeKB, featureCount, features }) => {
        ...
        features.forEach((feature, index) => {
            ...
            console.log(`${(index + 1).toString().padStart(2)}. ${feature.name.padEnd(25)} ${feature.sizeKB.toString().padStart(8)}KB (${percentage}%)`);
        });
    });
    ...
    // Summary
    console.log(`\n=== Summary ===`);
    console.log(`Largest feature: ${results.largestFeature.name} at ${results.largestFeature.sizeKB}KB on ${results.largestFeature.platform}`);
