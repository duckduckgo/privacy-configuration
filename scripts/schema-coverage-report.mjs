...
function getPlatformSpecificFeatures() {
    ...
    try {
        ...
        for (const [featureName, platforms] of allFeatures) {
...
function analyzeSchemaCoverage() {
    ...
    if (featuresWithSchema.size > 0) {
        ...
        Array.from(featuresWithSchema).sort().forEach(feature => {
            console.log(`   - ${feature}`);
        });
        console.log('');
    }
    ...
    if (platformSpecificFeatures.size > 0) {
        ...
        Array.from(platformSpecificFeatures).sort().forEach(feature => {
